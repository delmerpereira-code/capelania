    ...
    switch(p.acao) {
      ...
      case 'mudarSenha':
        ...
        for(var mi=1; mi<datMud.length; mi++){
          ...
          if(mat2 !== p.matricula) continue;
          var senhaAtual = (datMud[mi][5]||mat2).toString().trim();
          if(senhaAtual !== p.senhaAtual){ r={erro:'Senha atual incorreta.'}; break; }
          shMud.getRange(mi+1, 6).setValue(p.novaSenha); // col F
          mudOk = true; r={status:'ok'}; break;
        }
        if(!mudOk && !r) r={erro:'Matrícula não encontrada.'};
        break;
      case 'resetSenha':
